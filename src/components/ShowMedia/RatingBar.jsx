    ...
    <div className='row-span-2 justify-items-center grid grid-cols-3'>
     <div className='like-container'>
       <div className='flex items-end h-full bg-gray-200 w-full rounded-full'>
        <div className='rounded-b-full rounded-t-full bg-green-200 w-full' style={getPercentages(like_count)}></div>  
        </div>
       <button><FontAwesomeIcon onClick={() => {handleRatingClick('like');}} className={toggleLike} icon={faFaceGrin} /> </button>
      </div>
      
      <div className='meh-container'>
       <div className='flex items-end h-full mr-2 bg-gray-200 rounded-full w-full'>
        <div className={`rounded-b-full rounded-t-full bg-yellow-200 w-full`} style={getPercentages(meh_count)}></div>
       </div>
       <button><FontAwesomeIcon onClick={() => {handleRatingClick('meh');}} className={toggleMeh} icon={faFaceMeh} /> </button>
      </div>
      <div className='dislike-container'>
        <div className='flex items-end h-full bg-gray-200 rounded-full w-full'>
          <div className={'rounded-b-full rounded-t-full bg-red-200 w-full'} style={getPercentages(dislike_count)}></div>
        </div>
        <button><FontAwesomeIcon onClick={() => {handleRatingClick('dislike');}} className={toggleDislike} icon={faFaceFrown} /> </button>
      </div>
    </div>
  );
}