...
export default function RatingBar(props) {
  const { dislikePercent, likePercent, mehPercent, mediaInteraction } = props;
  const [dummyState, setDummyState] = React.useState(33)
    ...
    <div className='row-span-2 justify-items-center grid grid-cols-3'>
     <div className='like-container'>
       <div className='flex items-end h-full bg-gray-200 w-full rounded-full'>
        <div className='rounded-b-full bg-green-200 w-full' style={{height: `${likePercent}%`}}></div>  
        </div>
      ...
      <div className='meh-container'>
       <div className='flex items-end h-full mr-2 bg-gray-200 rounded-full w-full'>
        <div className={`rounded-b-full bg-yellow-200 w-full`} style={{height: `${mehPercent}%`}}></div>
       </div>
      ...
      <div className='dislike-container'>
        <div className='flex items-end h-full bg-gray-200 rounded-full w-full'>
          <div className={'rounded-b-full bg-red-200 w-full'} style={{height: `${dislikePercent}%`}}></div>
        </div>
