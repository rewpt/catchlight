...
export default function BottomPop(props) {
  ...
  const toggleLike = classNames({
    "like-btn-sml-clicked rating-face-popout": interactionButton === "like",
    "like-btn-sml": interactionButton !== "like",
  });
  const toggleMeh = classNames({
    "meh-btn-sml-clicked rating-face-popout": interactionButton === "meh",
    "meh-btn-sml": interactionButton !== "meh",
  });
  const toggleDislike = classNames({
    "dislike-btn-sml-clicked rating-face-popout":
  ...
  const postMediaButtonClick = (rating) => {
    axios
      .post("/api/interactions", { rating, mediaID }, jwt)
  ...
  useEffect(() => {
    ...
    axios
      .get(`/api/media/${mediaID}/interactions/`, jwt)
      .then((res) => {
        if (cancel) return;
      ...
      <div className="flex justify-end min-w-[60px] min-h-[22px] max-w-[60px] max-h-[22px] overflow-visible items-center">
        <button className="flex">
          <FontAwesomeIcon
            ...
            icon={faFaceGrin}
            onClick={() => postMediaButtonClick("like")}
          />{" "}
        </button>
        <button className="flex">
          <FontAwesomeIcon
            className={toggleMeh}
            icon={faFaceMeh}
            onClick={() => postMediaButtonClick("meh")}
          />{" "}
        </button>
        <button className="flex">
          <FontAwesomeIcon
            ...
            icon={faFaceFrown}
            onClick={() => postMediaButtonClick("dislike")}
          />{" "}
        </button>
      </div>
    </div>
